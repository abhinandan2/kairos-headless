...
function writeLog(automationID,instance,data) {
	...
	fs.appendFile(automator.testDirLogs + "/" + automationID + "#" + instance  + ".log",moment().format() + " --- " + data + '\n', 'utf8'
		,
    // callback function
    function(err) { 
        if (err){
...
function screenShotPath(automationID,instance,type){
	...
	return   automator.testDirScreenShots + "/" + automationID + "#" + instance  + "/"  +  moment().format() + "---" + type + ".jpg";
